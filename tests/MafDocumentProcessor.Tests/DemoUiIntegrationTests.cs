...
public sealed class DemoUiIntegrationTests
{
    ...
    public async Task Index_ExposesIndividualAndAccessibleCaptureModes()
    {
        ...
        Assert.Contains("/capture-ui.js?v=20260828.1", html, StringComparison.Ordinal);
        Assert.Contains("/app.js?v=20260828.1", html, StringComparison.Ordinal);
    }
    ...
    public async Task UiAssets_ExposeEphemeralRegionCorrectionAndReprocessing()
    {
        ...
        Assert.Contains("Document reference", app, StringComparison.Ordinal);
        Assert.Contains("referenceInput.maxLength = 128", app, StringComparison.Ordinal);
        Assert.Contains("region.sourceId = referenceInput.value", app, StringComparison.Ordinal);
        Assert.Contains("Your corrections are still available", app, StringComparison.Ordinal);
    }
