...
Region validation is deterministic. It converts untrusted `ProposedNormalizedBounds` into `NormalizedBounds` only when coordinates are finite, contained in the `0`–`1` image space, and above the configured useful-region thresholds. It then maps those bounds onto the oriented source by rounding opposite edges independently and rejects empty pixel crops. For detector proposals, it orders remaining regions top-to-bottom then left-to-right, drops near-duplicates at `DuplicateIntersectionOverUnionThreshold`, expands each accepted box by `RegionEdgePadding` on every side (clamped to the image), and records `detected regions overlap` when distinct retained regions exceed `OverlapReviewIntersectionOverUnionThreshold`. For caller overrides, it preserves every valid rectangle exactly and in submitted order; it does not pad, reorder, deduplicate, or add detector overlap warnings. Both paths cap accepted members at the smaller of `MaxDetectedRegionsPerSource` and `MaxMembersPerCapture`. A little neighbouring paper in an automatically detected crop is acceptable: classification and extraction are instructed to use the main document occupying most of the image, including its centre. Invalid, empty, and overflow regions become `invalid_detected_region` results; automatic duplicates do too. A successful detection or override that yields no accepted crop becomes `no_usable_document_region`.

`DocumentRegionDetection` is a separate configured model role. It makes one semantic call for each source without an override that passes declared-type, extension, byte-size, decoded-format, and dimension checks. A source with an override is still decoded and orientation-normalized, but its caller-supplied proposals replace detector output and contribute no detection model usage. Detection and override proposals share geometry, crop, useful-area, and member-limit checks; padding, spatial ordering, duplicate removal, and overlap review apply only to detector output. The detector returns only bounds, an optional four-point outline, and advisory confidence. The parser preserves numeric out-of-range proposals for the deterministic validator rather than treating model coordinates as trusted geometry.
...
| `RegionEdgePadding` | 0.03 | Extra normalized margin added on every side of an accepted detector crop; caller overrides are exact |
